...
def send_email(alert_message, is_alert, timestamp_str, smtp_password, from_address, to_address, station, alert_mode):
    if is_alert or alert_mode == 'INFO':
...
if __name__ == "__main__":
    if len(sys.argv) != 7:
        print("Usage: aprsStaleCheck.py <station> <api_key> <smtp_password> <from_address> <to_address> <alert_mode>")
        exit(1)
    ...
    if 'entries' in data and len(data['entries']) > 0:
        ...
        print(message)
        send_email(message, alert, formatted_lasttime, smtp_password, from_address, to_address, station, alert_mode)
    else:
        print(f"No data returned for station {station}.")
        message = f"No data returned for station {station}. Please verify the station id."
        send_email(message, True, '', smtp_password, from_address, to_address, station, alert_mode)